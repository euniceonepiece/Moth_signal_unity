...
namespace DiasGames.ThirdPersonSystem
{
    ...
    public class ThirdPersonSystem : MonoBehaviour
    {
        ...
        public GameObject LeftHand, RightHand, ThrowObj, ThrowHand;
        ...
        public bool Chokehold;
        public bool IsReadyThrow;
        ...
        {
            if (IsReadyThrow == true) {
                ThrowObj.GetComponent<BoxCollider>().isTrigger = false;
        ...
        public void ObjectThrowOver()
        {
            IsReadyThrow = false;
            Player_animator.SetLayerWeight(Player_animator.GetLayerIndex("ThrowObject"), 0);
        }
        public void GetObjectThrow()
        {
            float ThrowDistance = Vector3.Distance(transform.position, ThrowObj.transform.position);
            if (ThrowDistance <= 2) {
                ThrowObj.GetComponent<BoxCollider>().isTrigger = true;
                IsReadyThrow = true;
                rb = ThrowObj.GetComponent<Rigidbody>();
                rb.isKinematic = true;
                rb.transform.parent = ThrowHand.transform;
                rb.transform.position = ThrowHand.transform.position;
                Debug.Log("拿物品");
            }   
        }
         ...
         void Update()
        {
            ...
            if (!m_IsAICharacter)
            {
                ...
                if (m_InputManager.zoomButton.IsPressed || m_AlwaysZoomCamera || (Chokehold==true))
                    TryZoom();
                else
                    isZomming = false;
            }
            ...
            else
            {
                ...
                Player_animator.SetLayerWeight(Player_animator.GetLayerIndex("chokehold"), 0);
                IKWeights = 0;
            }
                // ----------------------- ABILITY UPDATE --------------------------- //
        ...
        void isChokehold()
        {
                Player_animator.SetLayerWeight(Player_animator.GetLayerIndex("chokehold"), 1.0f);
                IKWeights = 1;
        }
        void OnAnimatorIK()
        {
            Player_animator.SetIKPosition(AvatarIKGoal.LeftHand, LeftHand.transform.position);
            Player_animator.SetIKPositionWeight(AvatarIKGoal.LeftHand, IKWeights);

            Player_animator.SetIKRotation(AvatarIKGoal.LeftHand, LeftHand.transform.rotation);
            Player_animator.SetIKRotationWeight(AvatarIKGoal.LeftHand, IKWeights);

            Player_animator.SetIKPosition(AvatarIKGoal.RightHand, RightHand.transform.position);
            Player_animator.SetIKPositionWeight(AvatarIKGoal.RightHand, IKWeights);

            Player_animator.SetIKRotation(AvatarIKGoal.RightHand, RightHand.transform.rotation);
            Player_animator.SetIKRotationWeight(AvatarIKGoal.RightHand, IKWeights);
        }
        ...
        public Transform OnGetEnemy()
        {
            ...
            float distance_min = 1000;    //主角與怪物的最近距離
            float distance = 0;            //當前怪物與主角的距離
            int id = 0;                    //與主角最近的怪物的編號
                                           //遍歷所有敵人,計算距離並比較
            for (int i = 0; i < Guard.Length; i++)
            {
                if (Guard[i].activeSelf == true)
                {
                    distance = Vector3.Distance(transform.position, Guard[i].transform.position);
                    if (distance < distance_min)
                    {
                        //找到一個更近的
                        distance_min = distance;
                        id = i;
                        GuardTarget = Guard[id];
                    }
                }
            }
            return Guard[id].transform;
        }

        public Transform OnGetThrow()
        {

            //獲取所有敵人
            GameObject[] Throw = GameObject.FindGameObjectsWithTag("Throw");
            float distance_min = 1000;    //主角與怪物的最近距離
            float distance = 0;            //當前怪物與主角的距離
            int id = 0;                    //與主角最近的怪物的編號
                                           //遍歷所有敵人,計算距離並比較
            ...
            {
                if (Throw[i].activeSelf == true)
                {
                    distance = Vector3.Distance(transform.position, Throw[i].transform.position);
                    if ((distance < distance_min)&&(IsReadyThrow == false))
                    {
                        //找到一個更近的
                        distance_min = distance;
                        id = i;
